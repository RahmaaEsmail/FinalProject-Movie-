...
export default function Movies() {
  ...
  {
  let {data}=await axios.get(`https://api.themoviedb.org/3/discover/movie?api_key=eba8b9a7199efdcb0ca1f96879b83c44&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=false&page=${pageNum}`)
           ...
           {trendingMovies.map((movie,index)=>
               ...
               <span className={`${style.rating} position-absolute end-0 top-0 bg-info text-white p-2 fs-5`}>{(movie.vote_average).toFixed(1)}</span>
                  ...
                  <div className={`${style.layer} d-flex justify-content-center align-items-center text-info`}><p>Click to show more details... </p></div>
                   ...
                   <div className='mt-3'><span className='float-start text-muted'>{movie.release_date}</span> <span className='float-end bg-dark text-white-50 rounded-2 p-1'>movie</span></div>
